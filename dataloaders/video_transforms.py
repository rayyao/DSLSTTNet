    ...
    def padding(self, sample):
        ...
        for elem in sample.keys():
            ...
            if elem == 'curr_img' or elem == 'curr_label':
                ...
                all_tmp = tmp
                for tmp in all_tmp:
                    tmp = cv2.copyMakeBorder(tmp, top_pad, bottom_pad, left_pad, right_pad, cv2.BORDER_CONSTANT, value=pad_value)
                    new_tmp.append(tmp)
                tmp = new_tmp
            else:
                tmp = cv2.copyMakeBorder(tmp, top_pad, bottom_pad, left_pad, right_pad, cv2.BORDER_CONSTANT, value=pad_value)
...
class BalancedRandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        new_w = w if new_w >= w else new_w  # 这两步就是将图片的h, w与提供的465进行比较，选取较小的
        ...
        while (not is_contain_obj) and (step < self.max_step):
            ...
            after_crop = []
            contains = []
            for elem in ([ref_label, prev_label] + curr_label):  # 之前选取了较小的h, w, 这里在较大的图片中随机裁剪出较小图的区域
...
class RandomScale(object):
    ...
    Args:
        ...
        short_edge: 类似于crop后最短边的长度至少为short_edge, 然后乘上scale [short_edge * scale]
    """
    ...
    def __call__(self, sample):
        ...
        for elem in sample.keys():
            ...
            if elem == 'prev_img' or elem == 'curr_img' or elem == 'ref_img':
                flagval = cv2.INTER_CUBIC  # 图像采用三次样条插值
            else:
                flagval = cv2.INTER_NEAREST  # label最近邻插值
