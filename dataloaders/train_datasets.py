...
class VSDTrain(Dataset):
    ...
    def get_ref_index_v2(self, seqname, lablist, min_fg_pixels=200, max_try=20, total_gap=0):
        ...
        for _ in range(max_try):
            ...
            if ref_index in bad_indices:
                continue
            ref_label = cv2.imread(os.path.join(self.label_root, seqname, lablist[ref_index]), cv2.IMREAD_GRAYSCALE)
    ...
    def get_image_label(self, seqname, imagelist, lablist, index):
        ...
        label = cv2.imread(os.path.join(self.label_root, seqname, lablist[index]), cv2.IMREAD_GRAYSCALE)
    ...
    def sample_sequence(self, idx):
        ...
        while (is_consistent is False and try_step < max_try):
            ...
            ref_label = np.uint8(ref_label / 255.)
            ...
            for curr_index in curr_indices:
                ...
                curr_label = np.uint8(curr_label / 255.)
...
class ViSha(VSDTrain):
    def __init__(self,
        ...
        image_root = os.path.join(root1, split[0], 'images')
        label_root = os.path.join(root2, 'labels')
