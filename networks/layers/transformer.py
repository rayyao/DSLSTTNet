...
import dslstt.networks.layers.combiner as seg_combiner
...
class LongShortTermTransformer(nn.Module):
    def __init__(self,
        ...
        for idx in range(num_layers):
            ...
            layers.append(block(d_model, self_nhead, att_nhead, dim_feedforward, droppath_rate, \
                                lt_dropout, st_dropout, droppath_lst, activation))
    ...
    def forward(self, tgt, long_term_memories, short_term_memories, curr_id_emb=None, self_pos=None, size_2d=None):
        output = self.emb_dropout(tgt)  # [900, 2, 256]
        ...
        for idx, layer in enumerate(self.layers):
            ...
            if self.return_intermediate:
                intermediate.append(output)  # [900, 2, 256]
...
class DualBranch(nn.Module):
    def __init__(self, num_layers=2, d_model=256, self_nhead=8, att_nhead=8, dim_feedforward=1024, emb_dropout=0.,
                 droppath=0.1, lt_dropout=0., st_dropout=0., droppath_lst=False, droppath_scaling=False,
                 activation="gelu", return_intermediate=False, intermediate_norm=True, final_norm=True):
        ...
        self.emb_dropout = nn.Dropout(emb_dropout, True)
        combiner = seg_combiner.Combiner(d_model)
        ...
        if final_norm:
            num_norms += 1
        self.decoder_norms = [_get_norm(d_model * 2, type='gn', groups=2) for _ in range(num_norms)] if num_norms > 0 else None
...
class LongShortTermTransformerBlock(nn.Module):
    ...
    def forward(self, tgt, long_term_memory=None, short_term_memory=None, curr_id_emb=None,
        ...
        # Self-attention
        _tgt = self.norm1(tgt)  # [900, 2, 256]
        q = k = self.with_pos_embed(_tgt, self_pos)  # qk加上了pos_emb
        v = _tgt
        tgt2 = self.self_attn(q, k, v)[0]  # [900, 2, 256]
        tgt = tgt + self.droppath(tgt2)  # [900, 2, 256]

        # Long-Short Term Attention
        _tgt = self.norm2(tgt)  # [900, 2, 256]

        curr_Q = self.linear_Q(_tgt)  # [900, 2, 256]
        curr_K = curr_Q  # [900, 2, 256]
        curr_V = _tgt  # [900, 2, 256]

        local_Q = seq_to_2d(curr_Q, size_2d)  # [2, 256, 30, 30]
        ...
        tgt2 = self.long_term_attn(curr_Q, global_K, global_V)[0]  # [900, 2, 256]
        tgt3 = self.short_term_attn(local_Q, local_K, local_V)[0]  # [900, 2, 256]
...
class LongShortTermTransformerBlockV2(nn.Module):
    ...
    def forward(self, tgt, long_term_memory=None, short_term_memory=None, curr_id_emb=None, self_pos=None, size_2d=(30, 30)):
...
class GatedPropagationModule(nn.Module):
    ...
    def forward(self,
        ...
        if self.droppath_lst:
            tgt_combiner1 = self.combiner(tgt2, tgt3)
            tgt_id_combiner1 = self.combiner(tgt_id2, tgt_id3)
            tgt = tgt + self.droppath(tgt_combiner1)
            tgt_id = tgt_id + self.droppath(tgt_id_combiner1)
        else:
            tgt_combiner2 = self.combiner(tgt2, tgt3)
            tgt_id_combiner2 = self.combiner(tgt_id2, tgt_id3)
            tgt = tgt + self.lst_dropout(tgt_combiner2)
            tgt_id = tgt_id + self.lst_dropout(tgt_id_combiner2)

        # Cross-attention
        _tgt = self.norm2(tgt)
        _tgt_id = self.id_norm2(tgt_id)
        q = u = v = k = torch.cat([_tgt, _tgt_id], dim=-1)
