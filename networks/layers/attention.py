...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, Q, K, V):
        ...
        # Linear projections
        if self.use_linear:
            Q = self.linear_Q(Q)  # [900, 2, 256]
            K = self.linear_K(K)  # [900, 2, 256]
            V = self.linear_V(V)  # [900, 2, 256]
        ...
        # Multi-head
        Q = Q.view(-1, bs, num_head, self.d_att).permute(1, 2, 0, 3)  # [2, 8, 900, 32]
        K = K.view(-1, bs, num_head, self.d_att).permute(1, 2, 3, 0)  # [2, 8, 900, 32]
        V = V.view(-1, bs, num_head, hidden_dim).permute(1, 2, 0, 3)  # [2, 8, 900, 32]

        # Multiplication
        QK = multiply_by_ychunks(Q, K, self.qk_chunks)  # [2, 8, 900, 900]
        if self.use_dis:
        ...
        # Restore shape
        outputs = outputs.reshape(-1, bs, self.d_model)  # [900, 2, 256]
...
class MultiheadLocalAttentionV2(nn.Module):
    def __init__(self,
        ...
        self.relative_emb_k = nn.Conv2d(self.d_att*self.num_head, num_head*self.window_size*self.window_size, kernel_size=1, groups=num_head)
        self.relative_emb_v = nn.Parameter(
            torch.zeros([self.num_head, d_model//self.num_head, self.window_size*self.window_size]))
    ...
    def forward(self, q, k, v):
        ...
        if self.qk_mask is not None and (h, w) == self.last_size_2d:
            ...
            memory_mask = torch.ones((1, 1, h, w), device=v.device).float()
            unfolded_k_mask = self.pad_and_unfold(memory_mask).view(1, 1, self.window_size*self.window_size, h*w)  # win-15，h-30
            qk_mask = 1 - unfolded_k_mask
            self.qk_mask = qk_mask

        relative_emb = self.relative_emb_k(q)  # conv(256, 1800, k1s1g1) [2, 1800, 30, 30]

        # Scale
        q = q / self.T

        q = q.view(-1, self.d_att, h, w)  # [16, 32, 30, 30]
        k = k.view(-1, self.d_att, h, w)  # [16, 32, 30, 30]
        v = v.view(-1, self.num_head, hidden_dim, h*w)  # [2, 8, 32, 900]

        relative_emb = relative_emb.view(n, self.num_head, self.window_size*self.window_size, h*w)  # [2, 8, 225, 900]

        if self.enable_corr:
            qk = self.correlation_sampler(q, k).view(n, self.num_head, self.window_size*self.window_size, h*w)
        else:
            unfolded_k = self.pad_and_unfold(k).view(n*self.num_head, hidden_dim, self.window_size*self.window_size, h, w)
            qk = (q.unsqueeze(2)*unfolded_k).sum(dim=1).view(n, self.num_head, self.window_size*self.window_size, h*w)
        if self.use_dis:
            qk = 2 * qk - self.pad_and_unfold(k.pow(2).sum(dim=1, keepdim=True)).view(n, self.num_head, self.window_size*self.window_size, h*w)

        qk = qk + relative_emb
        qk -= qk_mask*1e+8 if qk.dtype==torch.float32 else qk_mask*1e+4

        local_attn = torch.softmax(qk, dim=2)
        local_attn = self.dropout(local_attn)  # [2, 8, 225, 900]
        agg_bias = torch.einsum('bhwn,hcw->bhnc', local_attn, self.relative_emb_v)  # [2, 8, 900, 32]

        global_attn = self.local2global(local_attn, h, w)  # [2, 8, 900, 900]

        agg_value = (global_attn @ v.transpose(-2, -1))  # [2, 8, 900, 32]
    ...
    def local2global(self, local_attn, height, width):
        ...
        global_attn = torch.zeros((batch_size, self.num_head, height * width, pad_height, pad_width), device=local_attn.device)
        ...
        global_attn = global_attn[:, :, :, self.max_dis:-self.max_dis,
                                  self.max_dis:-self.max_dis].reshape(batch_size, self.num_head, height*width, height*width)

        return global_attn

    def pad_and_unfold(self, x):
        pad_pixel = self.max_dis * self.dilation  # 7
        x = F.pad(x, (pad_pixel, pad_pixel, pad_pixel, pad_pixel), mode='constant', value=0)  # [1, 1, 44, 44] # 左右上下进行填充7个像素
        x = F.unfold(x, kernel_size=(self.window_size, self.window_size), stride=(1, 1), dilation=self.dilation)
        return x
...
class GatedPropagation(nn.Module):
    ...
    def forward(self, Q, K, V, U, size_2d):
        ...
        # Weighted sum
        if self.top_k > 0:
            TOP_V, indices = torch.topk(V, k=self.top_k, dim=-1)
            V = torch.zeros_like(V).scatter_(-1, indices, TOP_V)
...
class LocalGatedPropagation(nn.Module):
    ...
    def forward(self, q, k, v, u, size_2d):
        ...
        if self.top_k > 0:
            TOP_qk, indices = torch.topk(qk, k=self.top_k, dim=-1)
            top_attn = linear_gate(TOP_qk, dim=-1)
            local_attn = torch.zeros_like(qk).scatter_(-1, indices, top_attn)
        else:
